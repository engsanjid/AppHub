...
export default function Installation() {
  ...
  const sorted = useMemo(() => {
    ...
    copy.sort((a, b) => sort === 'asc' ? a.downloads - b.downloads : b.downloads - a.downloads);
    return copy;
  }, [apps, sort]);
  ...
  return (
    <section className="bg-[#f7f9fb] px-[24px] md:px-[80px] py-10">
      <div className='text-black flex flex-col justify-center items-center text-center gap-[12px]'>
        <h1 className="font-bold text-[32px] md:text-[48px] leading-tight">Your Installed Apps</h1>
        <p className="text-sm md:text-base text-gray-600">Manage installed apps stored in your browser</p>
      </div>

      <div className='flex flex-col md:flex-row items-start md:items-center justify-between gap-3 mt-6'>
        <h2 className="font-semibold text-[#001931]">({apps.length}) Apps Found</h2>
...
>
  ...
  <button
    ...
    className="btn btn-success text-white rounded-xl px-5"
  >
    Uninstall
  </button>
</div>
